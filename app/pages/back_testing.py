...
for year in st.session_state['state'].get_years():
    st.session_state["data"].load_result(
            ...
            columns=["index", f"{year}_{st.session_state['state'].get_type(as_type='code')}_pred", f"{year}_{st.session_state['state'].get_type(as_type='code')}_true"],
    )
    all_results.append(
...
pblocs = ['p'+b for b in st.session_state['state'].get_blocs(as_type='code', order='political')]
...
selected_blocs = st.multiselect(
    ...
    default=st.session_state['state'].get_blocs(as_type='code', order='political', prefix='p'),
)

plot_backtest(temporal_data, selected_blocs, years=st.session_state['state'].get_years())
