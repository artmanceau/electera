...
def _log_numeric_metrics(
    trend: str, values: dict, model_name: str, suffix: str = ""
) -> None:
    if not isinstance(values, dict):
        return
    for metric_name, metric_value in values.items():
        if isinstance(metric_value, (int, float, np.number)) and np.isfinite(
            metric_value
        ):
            mlflow.log_metric(
                f"{model_name}_{trend}_{metric_name}_{suffix}", float(metric_value)
            )
