...
class ElectionPredictor:
    ...
    def predict_votes(self, X, predict_delta=False, infer_multiple=False, agg=False):
        ...
        def prediction_fn(model, X, predict_delta, infer_multiple):
            ...
            if predict_delta:
                return preds + X[f"previouspvote{trend}"].fillna(
                    X[f"previouspvote{trend}"].mean()
                )
            else:
                return preds
        ...
        assert (
            np.abs(
                X_result[
                    ["pvote" + trend for trend in self.trends if trend != "par"]
                ].sum(axis=1)
                - 1
            ).mean()
            < 0.001
        ...
        if "inscrits" in X.columns:
            X_result["votants"] = (X_result["pvotepar"] * X_result["inscrits"]).astype(
                int
            )
            for trend in self.trends:
                if trend != "par":
                    X_result[f"vote{trend}"] = round(
                        X_result["pvote" + trend] * X_result["votants"], 0
                    ).astype(int)

        assert (
            np.abs(
                X_result[
                    [f"vote{trend}" for trend in self.trends if trend != "par"]
                ].sum(axis=1)
                - X_result["votants"]
            ).mean()
            < 0.5
        ...
        if agg:
            ...
            agg_results["tot_ppar"] = (
                agg_results["tot_par"] / X_result["inscrits"].sum()
            )
            for trend in self.trends:
                if trend != "par":
                    agg_results[f"tot_vote{trend}"] = X_result[f"vote{trend}"].sum()
                    agg_results[f"tot_pvote{trend}"] = (
                        X_result[f"vote{trend}"].sum() / X_result["votants"].sum()
                    )
    ...
    def compute_agg_results(X: pd.DataFrame, blocs: list, election_code: str):
        ...
        result.loc["exprimes_", f"{election_code}_true"] = (
            X[[f"vote{trend}_true" for trend in blocs]].sum().sum()
        )
        result.loc["pvoteexpr", f"{election_code}_true"] = round(
            X["exprimes"].sum() / X["inscrits_true"].sum() * 100, 2
        )
        ...
        result.loc["pvoteexpr", f"{election_code}_pred"] = result.loc[
            "pvotepar", f"{election_code}_pred"
        ]
        ...
        for x in [
            ...
            if x == "votants":
                result.loc[x, f"{election_code}_diff"] = round(
                    X[f"{x}_diff"].mean() * 100, 0
                )
                result.loc[x, f"{election_code}_std"] = round(
                    X[f"{x}_diff"].std() * 100, 0
                )
            if x == "pvotepar":
                result.loc[x, f"{election_code}_diff"] = round(
                    X[f"{x}_diff"].mean() * 100, 2
                )
                result.loc[x, f"{election_code}_std"] = round(
                    X[f"{x}_diff"].std() * 100, 2
                )

        result.loc["inscrits", f"{election_code}_diff"] = 0
        result.loc["inscrits", f"{election_code}_std"] = 0
        for x in ["exprimes", "exprimes_"]:
            result.loc[x, f"{election_code}_diff"] = result.loc[
                "votants", f"{election_code}_diff"
            ]
            result.loc[x, f"{election_code}_std"] = result.loc[
                "votants", f"{election_code}_diff"
            ]

        result.loc["pvoteexpr", f"{election_code}_diff"] = result.loc[
            "pvotepar", f"{election_code}_diff"
        ]
        result.loc["pvoteexpr", f"{election_code}_std"] = result.loc[
            "pvotepar", f"{election_code}_diff"
        ]
        ...
        for b in blocs:
            ...
            result.loc[f"vote{b}", f"{election_code}_diff"] = round(
                X[f"vote{b}_diff"].mean(), 0
            )
            result.loc[f"vote{b}", f"{election_code}_std"] = round(
                X[f"vote{b}_diff"].std(), 0
            )
            ...
            result.loc[f"pvote{b}", f"{election_code}_diff"] = round(
                X[f"pvote{b}_diff"].mean() * 100, 2
            )
            result.loc[f"pvote{b}", f"{election_code}_std"] = round(
                X[f"pvote{b}_diff"].std() * 100, 2
            )
    ...
    def compute_votes_per_circo(self, X):
        ...
        plm.loc[
            ...
            ["inscrits", "votants"]
            + [f"vote{trend}" for trend in self.trends if trend != "par"],
        ] = X[X["codecommune"].str.startswith("751")][
            ["inscrits", "votants"]
            + [f"vote{trend}" for trend in self.trends if trend != "par"]
        ].sum()
        plm.loc[
        ...
        plm.loc[
            ...
            ["inscrits", "votants"]
            + [f"vote{trend}" for trend in self.trends if trend != "par"],
        ] = X[X["codecommune"].str.startswith("132")][
            ["inscrits", "votants"]
            + [f"vote{trend}" for trend in self.trends if trend != "par"]
        ].sum()
        ...
        communes_with_no_circo = X_merged[X_merged["circo"].isna()][
            "codecommune"
        ].to_list()
        communes_with_no_circo_other_than_plm = list(
            set(communes_with_no_circo)
            - set(
                [
                    "13201",
                    "13202",
        ...
        if len(communes_with_no_circo_other_than_plm) > 0:
            logger.warning(
                f"Commune with no circo (other than PLM) : {communes_with_no_circo_other_than_plm}"
            )

        trends = [f"vote{trend}" for trend in self.trends if trend != "par"] + [
            "votants"
        ]
