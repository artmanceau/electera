...
from electera.components.explanability.counterfactuals.cf_data_processing import (
    CounterfactualDataProcessing,
)
from electera.components.explanability.counterfactuals.cf_generator.generator import (
    CounterfactualGenerator,
)
from electera.components.utils.config import CFConfig
...
class CounterfactualPipeline:
    ...
    def run(self, codecommune, variation=0.2):
        ...
        data = DataLoader.load_dataset(
            ...
            formate="parquet",
            columns=None,
            filters=[("annee", "==", float(self.year)), ("type", "==", self.t)],
        )

        # 1. Counterfactual generation preparation
        stat_computer = CounterfactualDataProcessing(
            self.model.models[self.var].features, self.cf_config
        )
        (
            metadata,
            mean_values,
        ...
        ) = stat_computer.compute_statistics(
            data[["codecommune"] + self.model.models[self.var].features]
        )
        ...
        generator = CounterfactualGenerator(
            model=model_object, config=self.cf_config, data_dict=data_dict
        )
        data_instance = data[
            (data["codecommune"] == codecommune)
            & (data["type"] == self.t)
            & (data["annee"] == float(self.year))
        ]
        ...
        if variation < 0:
            desired_ranges_scenario = (
                [-abs_variation, 0],
                f"Variation in the vote of -{abs_variation}%",
            )
        else:
            desired_ranges_scenario = (
                [0, abs_variation],
        ...
        counterfactuals_list = generator.generate_counterfactuals(
            query_instances=data_instance[self.model.models[self.var].features].copy(
                deep=True
            ),
...
if __name__ == "__main__":
    pipeline = CounterfactualPipeline(
        ...
        year=2022,
        type_="pres",
        vars_="['voteG', 'voteD', 'voteCG', 'voteCD', 'voteC', 'par']",
        version="0.0.1.",
