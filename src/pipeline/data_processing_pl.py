...
class ElectionDataProcessor:
    ...
    def load_communes_data(self):
        ...
        communes = pl.scan_csv('s3://arthurmanceau/election_modeling_uhcp/data/raw/insee_geo/2026/communes_2026.csv', infer_schema_length=40000).collect()
        ...
        communes = (
            ...
            .join(
                geo_data.select(['codecommune', 'lat', 'long'])
                        .rename({'lat': 'lat_parent', 'long': 'long_parent'}),
    ...
    def add_geographical_data(self):
        ...
        geo_data = geo_data.with_columns(
                lat=pl.col('lat').round(4),
                long=pl.col('long').round(4)
        )
        return geo_data
    ...
    def load_electoral_data(self):
                ...
                for root, _, files in xs.walk(os.path.join(folder_path, dir)):
                    for file in files:
                        if (
                            ...
                            election_type, year, election_code = self.interpret_election_path(os.path.relpath(root, folder_path))
                            ...
                            df = df.with_columns(
                                ...
                                codecommune=pl.col("codecommune").cast(pl.String).str.zfill(5),
                            )
                            df = df.cast({key: value for key, value in self.electoral_schema.items() if key in df.columns})
        ...
        # Further cleaning
        political_vote_cols = ["voteG", "voteCG", "voteC", "voteCD", "voteD", "voteTG", "voteTD","voteGCG", "voteDCD", "voixOUI", "voixNON"]
        general_vote_cols = ["inscrits", "votants", "exprimes"]
        vote_cols = political_vote_cols+general_vote_cols
        p_cols = ["pvotepar", "pvoteabs"] + ['p'+col for col in political_vote_cols]

        electoral_data = electoral_data.filter((
            pl.col('codecommune').str.slice(0, 2).is_between(pl.lit("01"), pl.lit("95")) |
            pl.col('codecommune').str.slice(0, 2).is_in(["2A", "2B"])
        )).filter(
            ~pl.all_horizontal(pl.col(vote_cols).is_null())
        ).with_columns(
            ...
            [
                ...
                .otherwise(pl.col(c))
                .alias(c)
                for c in ["votants", "exprimes"]+political_vote_cols
            ] + [
                pl.when(pl.col("inscrits") == 0)
                .then(pl.lit(True))
            ...
            [
                ...
                .otherwise(pl.col(c))
                .alias(c)
                for c in ["exprimes"] + political_vote_cols
            ] + [
                pl.when(pl.col("votants") == 0)
                .then(pl.lit(True))
                .otherwise(pl.lit(False))
            ...
            pl.any_horizontal(*[pl.col(c) >= 0 for c in vote_cols if c in electoral_data.columns])
        ).with_columns(
        ...
        assert electoral_data.filter(~(pl.col('election_code').is_in(['muni2026']))).filter(pl.col('inscrits').round(0) < pl.col('votants').round(0)).height == 0
        assert electoral_data.filter(~(pl.col('election_code').is_in(['muni2020', 'muni2026', 'muni2014', 'leg2024']))).filter(pl.col('exprimes_').round(0) != pl.col('exprimes').round(0)).height == 0
        ...
        electoral_data = electoral_data.with_columns(
            ...
            pl.col(p_cols).round(2)
        )
        assert electoral_data.filter(
            pl.any_horizontal(
                *[
                        ...
                        for file in files:
                            if (
                                ...
                                df_long = df.unpivot(
                                    ...
                                    annee=pl.col("variable").str.tail(4).cast(pl.Int32),
                                    feature_name=pl.col("variable").str.head(-4),
                                ).sort(
                                    [key, "feature_name", "annee"]
                                ).with_columns(
                                ...
                                float_cols = [c for c, dtype in zip(df_long.columns, df_long.dtypes) if dtype in (pl.Float32, pl.Float64)]
                                ...
                                df_long = df_long.cast({key: value for key, value in self.socio_economic_schema.items() if key in df_long.columns})
                                df_long = df_long.match_to_schema(
                                    self.socio_economic_schema,
                                    missing_columns="insert",
                                    extra_columns="ignore",
                                )
                                combined = df_long if combined is None else combined.vstack(df_long)
                
        socio_economic_data = combined.rechunk().fill_nan(None)
        assert socio_economic_data.drop('codecommune', 'feature_name', 'variable').select(pl.all().is_nan().sum()).sum_horizontal().item() == 0
        assert socio_economic_data.drop('codecommune', 'feature_name', 'variable').select(pl.all().is_infinite().sum()).sum_horizontal().item() == 0

        catalog = (
            socio_economic_data
            .group_by('feature_name')
            .agg(pl.col('annee').unique().sort())
        )

        return socio_economic_data, catalog

    @staticmethod
    def get_features_for_years(X, year=2022, feature_aug=["raw", 'lag', 'rank', 'delta', 'pct_change'], n_years_back=0):
        index_cols = ["codecommune"]

        # Base: pivot for the election year
        data = (
            X.filter(pl.col('annee') == year)
            .pivot(on="feature_name", index=["annee", "codecommune"], values=feature_aug, aggregate_function="first")
            .sort('codecommune')
        )
        ...
        for i in range(1, n_years_back + 1):
            ...
            new_cols = [c for c in df_back.columns if c not in data.columns and c not in index_cols]

            data = data.join(
                df_back.select(index_cols + new_cols).rename({c: f"{c}|minus_{i}" for c in new_cols}),
                on=index_cols,
                how="left"
            )
    ...
    def create_dataset(self, election_code, electoral_data, commune_data, socio_economic_data):
        political_vote_cols = ["voteG", "voteCG", "voteC", "voteCD", "voteD", "voteTG", "voteTD","voteGCG", "voteDCD", "voixOUI", "voixNON"]
        general_vote_cols = ["inscrits", "votants", "exprimes"]
        vote_cols = political_vote_cols+general_vote_cols
        p_cols = ["ppar", "pabs"] + ['p'+col for col in political_vote_cols]
        meta_cols = ['dep','nomdep','codecommune','nomcommune','annee','type','election_code','no_inscrits','no_votants','exprimes_', 'TYPECOM', 'NCC', 'NCCENR', 'LIBELLE'] + vote_cols + p_cols

        election_results = electoral_data.filter(pl.col('election_code')==election_code)
        
        # Merge with commune data
        dataset = election_results.join(commune_data.select('TYPECOM', 'codecommune', 'NCC', 'NCCENR', 'LIBELLE', 'lat', 'long'), on='codecommune', how='left')

        year = int(election_code[-4:])
        X = self.get_features_for_years(socio_economic_data, year=year, feature_aug=["raw", 'lag', 'rank', 'delta', 'pct_change'], n_years_back=0)
        dataset = dataset.join(X.drop('annee'), on='codecommune', how='left').sort('codecommune')
        ...
        cols_with_variation = [
            col for col, std_val in zip(std_df.columns, std_df.row(0))
        ...
        # Apply the filter
        dataset = dataset.select(list(set(meta_cols).intersection(set(dataset.columns))) + cols_with_variation)

        assert dataset.drop(cs.string() | cs.boolean()).select(pl.all().is_nan().sum()).sum_horizontal().item() == 0
        assert dataset.drop(cs.string() | cs.boolean()).select(pl.all().is_null().sum()).sum_horizontal().item() == 0
        assert dataset.drop(cs.string() | cs.boolean()).select(pl.all().is_infinite().sum()).sum_horizontal().item() == 0
    ...
    for election_type in processor.config.include_elections_of_type:
        ...
        for year in relevant_years:
            if int(year) >= processor.config.include_elections_after:
                ...
                dataset = processor.create_dataset(election_code, electoral_data, commune_data, socio_economic_data)
                agg_dataset = dataset if agg_dataset is None else pl.concat([agg_dataset, dataset], how = "diagonal")
