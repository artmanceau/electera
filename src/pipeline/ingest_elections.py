...
election_to_ingest = {
    ...
    "2020_muni": {
        "LINK": {
            0: "ingestion/municipales/2020/2020_1000_plus.csv",
            1: "ingestion/municipales/2020/2020_1000_moins.csv",
        },
    ...
    "2026_muni": {
        "LINK": {
            0: "ingestion/municipales/2026/2026.csv",
            1: "ingestion/municipales/2026/2026_plm.csv",
        },
    ...
    "2026_muni_t2": {
        "LINK": {
            0: "ingestion/municipales/2026/2026_t2.csv",
            1: "ingestion/municipales/2026/2026_plm_t2.csv",
        },
    ...
    "2014_muni": {
        "LINK": {
            0: "ingestion/municipales/2014/2014_1000_moins.csv",
            1: "ingestion/municipales/2014/2014_1000_plus.csv",
        },
    ...
    "2020_muni_t2": {
        "LINK": {
            0: "ingestion/municipales/2020/2020_1000_plus_t2.csv",
            1: "ingestion/municipales/2020/2020_1000_moins_t2.csv",
        },
...
class ElectionIngester:
    ...
    def download_open_and_delete_file(
        self,
        url,
        folder_path="data/raw/temp",
        file_name="election_temp",
    ):
        if self.source == "API":
                ...
                if pd.notna(nuance) and nuance != "nan":
                    if nuance in nuance_seen:
                        matches = [
                            ...
                            if item["codecommune"] == codecommune
                            and item["trend"] == nuance
                        ]
    ...
    def apply_political_adjustments(self, X):
        X = X.copy()
        trends = X.isna().astype(int).sum(axis=0).sort_values().index.to_list()
        ...
        X["nb_list"] = (
            X[list(set(X.columns) - set(["codecommune"] + vote_nan_cols))]
            .notna()
            .sum(axis=1)
            / 2
        )
        ...
        X = X[
            ["codecommune"] + trends_kept + ["vote_AUTRES", "pvote_AUTRES", "nb_list"]
        ]
        X = X.fillna(0.0)
        X[trends_kept + ["vote_AUTRES", "pvote_AUTRES"]] = X[
            trends_kept + ["vote_AUTRES", "pvote_AUTRES"]
        ].astype(float)

        X = X.loc[~(X[trends_kept_p + ["pvote_AUTRES"]].sum(axis=1) == 0), :]

        if (
            ~(np.abs(X[trends_kept_p + ["pvote_AUTRES"]].sum(axis=1) - 100) < 2)
        ).astype(int).sum() != 0:
            logger.warning(
                f"These communes are outliers: {X.loc[(~(np.abs(X[trends_kept_p + ['pvote_AUTRES']].sum(axis=1) - 100) < 2)), :]['codecommune'].to_list()}"
            )
            X = X.loc[
                (np.abs(X[trends_kept_p + ["pvote_AUTRES"]].sum(axis=1) - 100) < 2), :
            ]
        ...
        for pol_trend in ["G", "D", "CD", "CG", "C"]:
            ...
            X[f"pvote{pol_trend}"] = (
                X[[col for col in trends_kept_p if col.split("_")[1] in partis]].sum(
                    axis=1
                )
                / 100
            )
    ...
    def run(self):
        ...
        assert (
            ~(
                np.abs(
                    data_merged["votants"] / data_merged["inscrits"]
                    - data_merged["ppar"]
                )
                < 0.01
            )
        ...
        assert (
            ~(
                np.abs(
                    data_merged["abstentions"] / data_merged["inscrits"]
                    - data_merged["pabs"]
                )
                < 0.01
            )
        ).astype(int).sum() == 0
        assert (~(np.abs(data_merged["ppar"] + data_merged["pabs"] - 1) < 0.01)).astype(
            int
        ).sum() == 0
