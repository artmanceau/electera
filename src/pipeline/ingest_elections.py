...
political_mapping = {
    ...
    "D": ["voix_RN", "pvoix_RN", "voix_REC", "pvoix_REC", "voix_LR", "pvoix_LR", "voix_DVD", "pvoix_DVD"],
    "CD": [],
...
class ElectionIngester:
    ...
    def download_open_and_delete_file(self, url, folder_path="data/raw/temp", file_name="election_temp.xlsx"):
    ...
    def get_and_rename(self, X, mapping={
        ...
        for idx, row in data.iterrows():
            ...
            for i in range(1, N+1):
                ...
                if pd.notna(nuance) and nuance != '':
                    ...
                    pivot_data.append({'codecommune': codecommune, 'trend': nuance, 'voix': voix, 'pvoix': pvoix})

        pivot_dataset = pd.DataFrame(pivot_data)
        
        pivot_dataset_grouped = pivot_dataset.groupby(['codecommune', 'trend'], dropna=False).agg({'voix': 'sum', 'pvoix': 'sum'}).reset_index()

        # # Pivot to wide format
        X = pivot_dataset_grouped.pivot(index='codecommune', columns='trend', values=['voix', 'pvoix'])
    ...
    def apply_political_adjustments(self, X):
        X = X.copy()
        trends_kept = X.isna().astype(int).sum(axis=0).sort_values()[1:23].index.to_list()
        ...
        X[trends_kept_p] = X[trends_kept_p].add(1-X[trends_kept_p].sum(axis=1), axis=0)

        for pol_trend in ['G', 'D', 'CD', 'CG', 'C']:
            partis = political_mapping[pol_trend]
            X[f'voix_{pol_trend}'] = X[[col for col in trends_kept_v if col.split('_')[1].isin(partis)]]
            X[f'pvoix_{pol_trend}'] = X[[col for col in trends_kept_p if col.split('_')[1].isin(partis)]]
    ...
    def save_processed_election(self, X):
        if not DataUtils._detect_s3(self.data_path):
            ...
            DataLoader.write_dataset(
                X,
                self.data_path+self.output_file
            )
        else:
            DataLoader.write_dataset(
               X,
